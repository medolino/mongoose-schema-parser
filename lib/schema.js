...
const _generateTimestampFiledsSchema = (options = {}) => {
  ...
  const fields = Object.entries({ createdAt, updatedAt }).reduce((schema, [timestampOptName, timestampParamName], ix) => {
    if (!timestampParamName) {
      return schema
    }

    // when timestamps.{timestampField} is string, split it to path and field value, otherwise use default property name
    const fieldPath = typeof timestampParamName === 'string' ? timestampParamName.split('.') : [timestampOptName]
...
const getPropertyType = (property) => {
  ...
  if (property.type && property.type.name) { // simple type
    // remove Schema from type name to extract simple type (introduced in mongoose 8)
    type = property.type.name.startsWith('Schema') ? property.type.name.replace('Schema', '') : property.type.name
...
const getMongooseArrayType = (arrayDetails) => {
  ...
  if (arrayDetails.length > 0) { // check if array content type is provided
    ...
    // if name is schema property, use constructor name and replace Schema to extract simple type (introduced in mongoose 8)
    let arrayContentType = (!isNameSchemaProperty && details.name) ? details.name.replace('Schema', '') : undefined
